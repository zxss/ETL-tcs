...
# ИЗМЕРЕНИЕ: на реплее штрафы УХУДШАЮТ результат во всех трёх режимах —
# у эвристики альфа падает с +2.1% до -2.8% годовых. Дефолт оставлен
# включённым, чтобы не менять поведение молча; отключать осознанно и
# перепроверять, поскольку сравнение шло по девяти вариантам на одной
# выборке и само подвержено переобучению на отбор.
APPLY_RISK_PENALTIES: bool = os.getenv("APPLY_RISK_PENALTIES", "1") not in ("0", "false", "False")
