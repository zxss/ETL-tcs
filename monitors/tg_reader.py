...
ВРЕМЕННО ОТКЛЮЧЕНО: все обращения к Telegram API закомментированы.
Для включения:
  1. Заполните TG_API_ID, TG_API_HASH, TG_PHONE в .env
  2. Раскомментируйте блоки с пометкой # TG_ENABLED
...
# ── TG_ENABLED: раскомментировать импорты Telethon ───────────────────────────
...
# ── TG_ENABLED: раскомментировать функцию _make_client ───────────────────────
...
async def auth() -> None:
    ...
    TG_ENABLED: раскомментировать тело функции.
    """
    log.warning("Telegram отключён. Раскомментируйте TG_ENABLED-блоки в tg_reader.py.")
...
async def fetch_messages(
    ...
    Возвращает список словарей: {message_id, ts (UTC datetime), text}

    TG_ENABLED: раскомментировать тело функции, убрать заглушку return [].
    """
    log.warning("Telegram отключён — возвращаем пустой список.")
    return []
